  ...
  def insert(self, c):
    ...
    killcounts_l = [(k,
                     ...
                     self.killer_stats[k][1]['name'])
                                        for k in list(self.killer_stats.keys())]
    ...
    killgames_del = [(ckiller,) for ckiller in list(self.killer_stats.keys())]
    killgames_ins = [self.killer_stats[k][1] for k in list(self.killer_stats.keys())]
...
class Streaks(BulkDBCache):
  ...
  def insert(self, c):
    ...
    cached_streaks.extend(list(self.cached_streaks.values()))
    ...
    last_games_del = [[name] for name in list(self.last_games.keys())]
    last_games_ins = [self.last_games[name] for name in list(self.last_games.keys())]
...
class PlayerStats(BulkDBCache):
  ...
  def insert(self, c):
    players_l = [[self.players[k]['name'], # use case from logfile, not key
                  ...
                  self.players[k]['max_runes']] for k in list(self.players.keys())]
    ...
    pl_char_l = [[k[0],
                  ...
                  self.pl_char[k][0],
                  self.pl_char[k][1],
                  self.pl_char[k][2]] for k in list(self.pl_char.keys())]
...
class PerDayStats(BulkDBCache):
  ...
  def insert(self, c):
    ...
    per_day_l = [[k,
                  ...
                  self.per_day_stats[k][1]] for k in list(self.per_day_stats.keys())]
    ...
    date_players_l = [[k[0],
                       ...
                       self.date_players[k][1]] for k in list(self.date_players.keys())]
